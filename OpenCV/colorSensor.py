...
# ref from
# https://opencv2-python-tutorials.readthedocs.io/en/latest/py_tutorials/py_imgproc/py_houghcircles/py_houghcircles.html
...
for img in circleList:
    ...
    # white
    # white vals ~ [250, 249, 246] - [255,255,255]
    elif int(rgb[0]) in range(250, 255) and int(rgb[1]) in range(240, 250) and int(rgb[2]) in range(200, 250):
        redTotal += 1
        cv2.circle(original, (current[0], current[1]), current[2], (0, 255, 0), 1)
    else:
        defaultTotal += 1

cv2.putText(original, ("Blue: " + str(blueTotal)), (225, 30), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 1,
            cv2.LINE_AA)

cv2.putText(original, ("Orange: " + str(orangeTotal)), (225, 60), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 1,
            cv2.LINE_AA)

cv2.putText(original, ("Yellow: " + str(yellowTotal)), (225, 90), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 1,
            cv2.LINE_AA)

cv2.putText(original, ("Brown: " + str(brownTotal)), (225, 120), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 1,
            cv2.LINE_AA)

cv2.putText(original, ("Green: " + str(greenTotal)), (225, 150), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 1,
            cv2.LINE_AA)

cv2.putText(original, ("Red: " + str(redTotal)), (225, 180), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 1,
            cv2.LINE_AA)

cv2.putText(original, ("White: " + str(redTotal)), (225, 250), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 1,
            cv2.LINE_AA)
